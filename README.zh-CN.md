...
在 Microsoft OneNote 中编写和渲染 Markdown 的插件。支持实时预览、LaTeX 公式、图表源码块和代码高亮，界面支持中英文切换。
...
| **LaTeX 公式** | 支持行内和块级数学公式渲染为图片 |
| **图表源码块** | 识别 Mermaid 等围栏代码并以保留格式显示源码 |
| **代码高亮** | 多种语言语法高亮，支持行号显示 |
| **导入/导出** | 导入 `.md` 文件渲染到页面，或将页面导出为 Markdown 文件 |
| **剪贴板支持** | 一键复制页面为 Markdown |
| **设置对话框** | 图形化配置字体、字号、LaTeX、行号及界面语言 |
